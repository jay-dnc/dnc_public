...
function ClickPolitMap(obj,evt) {
	...
	var n = PolitMap.length;
	var id = 0;
	if(n > 1){
		var mapSize = PolitMap[0];
		var cury = 1, curx = 0;
		var i = 1;
		do {
			var ix = PolitMap[i++];
			...
			else if(ix-- > curx && ix < ((cury < y) ? mapSize : x)){
				curx = ix;
				id = PolitMap[i++];
			}
			else {
	...
	if(place) {
		place.innerHTML=StrFoundOnPolitMap1+x+':'+y+'<br/>'+(Kingdoms[id] ? StrFoundOnPolitMap2 : StrFoundOnPolitMap3);
	}
