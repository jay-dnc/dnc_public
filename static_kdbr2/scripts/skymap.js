...
// True if selecting fleets, false if panning the map
var selectFleetsModeOn = false;
...
//	alert((CacheSize*2+CellsX)+':'+(CacheSize*2+CellsY)+"\n"+TableWidth+':'+TableHeight);
	...
	tacticalmap.dragEnabled = !selectFleetsModeOn;
}
...
function RefreshFleetsBtn(X, Y) {
	...
	if(pl){
		var FleetsBtn = DrawFleetsBtn(X, Y, ZoomIndex);
...
function TogglePlanetInfo(X, Y, show){
	...
	if (PlanetInfo.tr) {
		// if refresh turn is known
		informerTitle += ' <span>(ход '+PlanetInfo.tr+')</span>';
	}
...
//	Out[Out.length] = ' <a href="javascript:blablabla('+X+','+Y+');" class="nounder small_spoiler_white" title="Сменить иконку">[⌂]</a>';
	...
	if(PlanetInfo.t) {
		...
		if (PlanetInfo.is == 0 && PlanetInfo.i > 90) {
			Out[Out.length] = 'Нет планеты.'
		}
		else {
	...
	var obj = InfoDiv.getElementsByTagName('td')[0];
	Drag.init(obj, InfoDiv);
	InfoDiv.onDragStart = PinInformer;
	obj.enableNegativePos = false;
...
function getPinnedInformerHeaderHtml(id, x, y, title) {
	return '<div style="background-color:#DDDDDD;border:0px;color:black;font-size:1em;width:2em;float:left;">' +
			...
			'<div style="width:27px;white-space:nowrap;padding-left:5px;padding-right:4px;float:right;text-align:right;">' +
//			'<a href="#" class="super_black">✔</a> ' +
			(x && y ? '<a href="javascript:ScrollMapTo('+x+','+y+')" onmouseover="BlinkItem('+x+','+y+')" class="super_black"><img src="'+StaticSiteName+'/img/black_dot.gif" alt="->" style="width:11px;height:11px;border:0px;margin-right:5px;"/></a>' : '') +
...
function printPlanetStats(x, y) {
	...
	res += '<span title="Население">' + planetStats.currentPopulation.toLocaleString() + '</span>' +
			'<span style="color:#FFBBFF;" title="Свободное"> (' + (planetStats.freePopulation > 0 ? '+' : '') + planetStats.freePopulation.toLocaleString() + ')</span>';
...
function DrawMyFleets(X, Y, FleetsCanFly, multiplePlanetsMode){
	var MyFleetsHere = window.parent.frames.flets_frame.FleetsByXY[X+':'+Y] || [];
	var MyOutgoingFleetsHere = window.parent.frames.flets_frame.OutgoingFleetsByXY[X+':'+Y] || [];
	var Out = [];
	var myFleetsBlockId = 'my_fleets_'+X+'_'+Y;

	Out[Out.length] = '<a style="font-size:120%;" class="small_spoiler_white" href="javascript:ExpandSmallBlock(\''+myFleetsBlockId+'\',null,'+X+','+Y+')">'
	Out[Out.length] = '<img src="'+StaticSiteName+'/img/collapse_small.gif" alt="" width="10" height="13" border="0" id="'+myFleetsBlockId+'_arrow"/> ';
	if (multiplePlanetsMode) {
		Out[Out.length] = X+':'+Y + ' - ';
	}
	else {
		Out[Out.length] = parent.GetLangMSG(637)+' - ';
	}
...
//	TODO: check width
	...
	if(!alien) {
		Out[Out.length] = '<td style="text-align:right;padding-right:'+(alien ? '0' : '3')+'px; min-width: 28px; width: 28px;">';
		Out[Out.length] = '<div style="white-space:nowrap; height: 15px;">';
		if(!Fleet.ta && Fleet.un.length && !Fleet.fa.length && !parent.Actions['f'+Fleet.i] && !Fleet.fa.a2 && !Fleet.fa.a1) {
			...
			Out[Out.length] = '<input id="fleet_chkbx_'+(Fleet.i)+'" type="checkbox" style="float: left; margin: 0px 0px 0px 2px; width: 13px; height: 14px;" class="fleet_chkbx">';
	...
	Out[Out.length] = '<td style="padding: 0px 1px; white-space:nowrap;" title="' + title + '">';
...
function InitFleetDragControl(Fleet, FleetsCanFly){
	...
	Dragger.enableNegativePos = false;
	var jumpInfo;
	var Planet;
...
//		console.log("DrugMouseY = %d", clientY);
				...
				informer.offsetTop < clientY && informer.offsetTop + informer.offsetHeight > clientY) {
					if(informer.scrollDir) {
						...
						if (informer.planet) {
							...
							// informer accepts fleets but have no planet.
							cancelJump = true;
						}
						top_zIndex = informer.style.zIndex;
					}
		...
		if(!Planet && !cancelJump) {
			Planet = WndToMapCoords(this.DrugMouseX, this.DrugMouseY);
		}
//		console.log("[x, y] = [%d, %d]", Planet.x, Planet.y);

		if((!Planet && !PrevPlanet) || (Planet && PrevPlanet && PrevPlanet.x == Planet.x && PrevPlanet.y == Planet.y)) {
			RePositionJumpInfo(this.DrugMouseX, this.DrugMouseY, jumpInfo);
			return;
		}
		...
		if (!cancelJump) {
			...
			jmpInfoHtml += '<span style="font-weight: bold; font-size: 110%;">'+EscapeName(PlanetData.n)+
			...
			for (var i = 0; i < FleetsList.length; i++) {
				...
				var sameFleets = 1;
				while (i + 1 < FleetsList.length && FleetsList[i + 1].n == f.n && FleetsList[i + 1].r == f.r && FleetsList[i + 1].sp == f.sp 
						&& FleetsList[i + 1].x == f.x && FleetsList[i + 1].y == f.y) {
					sameFleets += 1;
					i += 1;
				}
...
function ImproveDncNow() {
	...
	enableColorBtn();
	upgradeGoToButton();
	enableFleetSelectionButton();
	
	document.getElementById('ToolPicker').width = 160;
	document.getElementById('ToolPicker').parentNode.colSpan = 6;
}
...
function enableFleetSelectionButton() {
	$('#ToolTable tr:last').append('<td id="selectFleetsBtn" align="center" style="" onclick="selectFleetsToggle();">'+
		'<img id="selectFleetsImg" width="20" height="20" border="0" title="Выбор флотов/Навигация" alt="S" src="'+StaticSiteName+'/img/pan_map.gif"/></td>');
}

var fleetSelectionInitialized = false;
function selectFleetsToggle() {
	if (selectFleetsModeOn) {
		document.getElementById('selectFleetsImg').src = StaticSiteName+'/img/pan_map.gif';
		$('#tablecontainer').dragToSelect('disable');
		document.getElementById('tacticalmap').dragEnabled = true;
	}
	else {
		document.getElementById('selectFleetsImg').src = StaticSiteName+'/img/select_fleets_off.gif';
		if (!fleetSelectionInitialized) {
			$('#tablecontainer').dragToSelect({
				selectables: 'div',
				percentCovered: 1,
				onHide: showFleetsOnSelectedPlanets
			});
			fleetSelectionInitialized = true;
		}
		$('#tablecontainer').dragToSelect('enable');
		
		document.getElementById('tacticalmap').dragEnabled = false;
	}
	selectFleetsModeOn = !selectFleetsModeOn;
}

function showFleetsOnSelectedPlanets() {
	var planetDivs = $('#MapTable div.selected');
	var selectedPlanets = []
	var FleetsCanFly = []
	var Out = []
	var minX = 9999999, minY = 9999999, maxX = 0, maxY = 0;
	planetDivs.each(function(index, planetDiv) {
		var coords = /navigator_cell_(\d+)_(\d+)/.exec(planetDiv.id);
		if (coords) {
			...
			var MyFleetsHere = window.parent.frames.flets_frame.FleetsByXY[X+':'+Y] || [];
			var MyOutgoingFleetsHere = window.parent.frames.flets_frame.OutgoingFleetsByXY[X+':'+Y] || [];
			if(MyFleetsHere.length || MyOutgoingFleetsHere.length) {
				selectedPlanets.push([X, Y]);
				
				Out[Out.length] = DrawMyFleets(X, Y, FleetsCanFly, true);
	...
	var informerId = 'fleets_on_planets_' + minX + '_' + minY + '_' + maxX + '_' + maxY;
	...
	var infromerTitle = 'Флоты <b>' + minX + ':' + minY + ' - ' + maxX + ':' + maxY + '</b>'
	var contentHtml = '<div style="max-height:'+(document.getElementById('tacticalmap').clientHeight - 100)+'px; overflow:auto; padding-right: 4px; min-width: 250px">' +
					Out.join('') + '</div>';

	CreateInformer(informerId, null, null, infromerTitle, contentHtml);
	
	//	init fleets drag controls
	for(var i = 0; i < FleetsCanFly.length; i++) {
		var Fleet = FleetsCanFly[i];
		InitFleetDragControl(Fleet, FleetsCanFly);
	}
...
function CreateInformer(id, x, y, title, content, className) {
	...
	if (!informer) {
		...
		var obj = informer.getElementsByTagName('td')[0];
		Drag.init(obj, informer);
		obj.enableNegativePos = false;
	}
	else{
...
if (document.location.pathname.match(/\/army/)) {
	...
	loadScript(StaticSiteName + '/scripts/jquery-1.11.1.min.js?v=1.11.1', 
		function() { loadScript(StaticSiteName + '/scripts/jquery.contextMenu.js?v=1.71',
			function() { loadScript(StaticSiteName + '/scripts/jquery.dragToSelect.js?v=1.1') }
			); });
	loadCss(StaticSiteName + '/jquery.contextMenu.css?v=1.71');
	loadCss(StaticSiteName + '/jquery.dragToSelect.css?v=1.1');
}