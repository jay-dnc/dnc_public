...
function FillChassisBonuses(Show){
		...
		if(nrid=='source'){
			...
			for(var i=1;i<BTNames.cells.length;i++){
				...
				if(itm[CurCell.id] && itm[CurCell.id]!=0){
					...
					NewValue.innerHTML=(val>0 ? '+' : '')+val;
					if(!CurCell.id.match('_abs_')) NewValue.innerHTML+='%';
	...
	if(!UnitSwitched){
		...
		if(!Aborted){
			...
			if(UnitID==-1){
				...
				if(!window.parent.FilledCells['sc'+ActiveSection]) window.parent.FilledCells['sc'+ActiveSection]=new Array();
				if(!window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1]){
					window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1]={nm:UnitID,lvl: Unit.transport_capacity ? 0 : Unit.maxlvl}
				}else{
					window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1].nm=UnitID;
					window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1].lvl=Unit.maxlvl<window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1].lvl ? Unit.maxlvl : window.parent.FilledCells['sc'+ActiveSection][ActiveCell-1].lvl;
				}
...
function CalculateShipData(Items,BldrRace,CalcImportPrices,CalcImportNeeds){
	var unit={support_main:0,support_second:0,weight:0,max_weight:0,req_tehn_level:1};
   ...
   var AllTDs=window.parent.unitsummary.document.getElementsByTagName('TD');
	...
	for(var i in unit){
		...
		if(ctrl){
			...
			ctrl.innerHTML=unit[i]?(Math.floor(unit[i]*rval)/rval):'-';
		}
	}
	...
	for(var i0=0;i0<Validate.length;i0++){
		...
		if(ctrl){
			...
			if(Validate[i0]=='weight'){
				...
				ctrl.innerHTML=RealWeight+'/'+RealMaxWeight;
